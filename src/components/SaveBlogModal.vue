...
import type { BlogPreview, SaveStatus } from '@/composables/useSaveBlog'

marked.setOptions({ breaks: true, gfm: true })

const props = defineProps<{
  saveStatus: SaveStatus
  preview: BlogPreview | null
...
const emit = defineEmits<{
  (e: 'save', data: BlogPreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()

const form = ref<BlogPreview | null>(null)
    ...
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      <div class="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        <div class="flex items-center gap-2.5">
          ...
          <h2 class="text-lg font-semibold text-foreground">Save Blog to Notebook</h2>
        </div>
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-xl font-semibold text-foreground">Blog post saved!</h3>
        <p class="text-sm text-muted-foreground text-center max-w-sm">Your sermon blog post has been saved to the Notebook.</p>
        <button @click="$emit('close')" class="mt-2 inline-flex items-center gap-2 rounded-md bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
          Done
        </button>
      </div>
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Preparing your blog post...</h3>
        <p class="text-sm text-muted-foreground">Pulling the blog post from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Could not find blog post</h3>
        <p class="text-sm text-muted-foreground text-center">{{ saveError || 'Make sure the blog post has been generated in the conversation.' }}</p>
      ...
      <div v-else-if="form" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <p class="text-sm text-muted-foreground">Review your blog post before saving. This was extracted from your conversation.</p>

        <div class="space-y-1.5">
          <label class="text-sm font-medium text-foreground">Blog Title *</label>
          <input
            v-model="form.title"
            type="text"
            placeholder="e.g. How to Find Peace When Life Feels Uncertain"
        ...
        <div class="space-y-1.5">
          ...
          <div class="rounded-lg border border-border bg-muted/30 p-4 max-h-80 overflow-y-auto prose prose-sm prose-slate max-w-none prose-headings:mt-3 prose-headings:mb-1 prose-h2:text-sm prose-h2:font-semibold prose-h3:text-xs prose-h3:font-semibold prose-p:my-1 prose-p:text-xs prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-strong:text-foreground">
            <div v-if="contentPreview" v-html="contentPreview" />
            <p v-else class="text-xs text-muted-foreground italic">No blog content found.</p>
          </div>
        </div>
      </div>
