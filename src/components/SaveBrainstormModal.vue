...
import type { BrainstormPreview, SaveStatus } from '@/composables/useSaveBrainstorm'

marked.setOptions({ breaks: true, gfm: true })

const props = defineProps<{
  saveStatus: SaveStatus
  preview: BrainstormPreview | null
...
const emit = defineEmits<{
  (e: 'save', data: BrainstormPreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()
...
onMounted(() => {
  try {
    ...
    console.error('[SaveBrainstormModal] Failed to load series:', err)
  }
})
...
const form = ref<BrainstormPreview | null>(null)
...
const canSave = computed(() =>
  form.value && form.value.passage.trim().length > 0 && form.value.content.trim().length > 0
)
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="$emit('close')">
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-xl font-semibold text-foreground">Brief saved!</h3>
        <p class="text-sm text-muted-foreground text-center max-w-sm">
          Your sermon brief has been saved to the Notebook.
        </p>
        <button
          @click="$emit('close')"
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Preparing your brief...</h3>
        <p class="text-sm text-muted-foreground">Pulling the sermon brief from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Could not find brief</h3>
        <p class="text-sm text-muted-foreground">{{ saveError || 'Make sure the conversation includes the Expanded Sermon Brief.' }}</p>
      ...
      <div v-else-if="form" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <p class="text-sm text-muted-foreground">
          Review your brief before saving. This was extracted from your conversation.
        </p>
        ...
        <div class="space-y-1.5">
          ...
          <p class="text-xs text-muted-foreground">Link this brief to an existing sermon series, or keep it standalone.</p>
        </div>

        <!-- Passage -->
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-foreground">Passage *</label>
          <input
            v-model="form.passage"
            type="text"
            placeholder="e.g. John 11:1-44"
        ...
        <!-- Big Idea -->
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-foreground">Big Idea</label>
          <input
            v-model="form.bigIdea"
            type="text"
            placeholder="One sentence summary of the core message"
            class="flex h-10 w-full rounded-md border border-input bg-card px-3 py-2 text-sm transition-colors placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          />
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="rounded-lg border border-border bg-muted/30 p-4 max-h-64 overflow-y-auto prose prose-sm prose-slate max-w-none prose-headings:mt-3 prose-headings:mb-1 prose-h2:text-sm prose-h2:font-semibold prose-h3:text-xs prose-h3:font-semibold prose-p:my-1 prose-p:text-xs prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-strong:text-foreground">
            ...
            <p v-else class="text-xs text-muted-foreground italic">No brief content found.</p>
          </div>
        </div>
      </div>
