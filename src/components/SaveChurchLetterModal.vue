...
import type { ChurchLetterPreview, SaveStatus } from '@/composables/useSaveChurchLetter'

marked.setOptions({ breaks: true, gfm: true })

const props = defineProps<{
  saveStatus: SaveStatus
  preview: ChurchLetterPreview | null
...
const emit = defineEmits<{
  (e: 'save', data: ChurchLetterPreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()

const form = ref<ChurchLetterPreview | null>(null)
    ...
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      <div class="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        <div class="flex items-center gap-2.5">
          ...
          <h2 class="text-lg font-semibold text-foreground">Save Church Letter</h2>
        </div>
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <p class="text-sm text-muted-foreground text-center max-w-sm">Your Church Letter was saved to the Notebook.</p>
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <p class="text-sm text-muted-foreground">Extracting final letter output from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <p class="text-sm text-muted-foreground text-center">{{ saveError || 'Make sure the final Church Letter output has been generated.' }}</p>
