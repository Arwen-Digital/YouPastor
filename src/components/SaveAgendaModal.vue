...
import type { AgendaPreview, SaveStatus } from '@/composables/useSaveAgenda'
...
const emit = defineEmits<{
  (e: 'save', data: AgendaPreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()

const form = ref<AgendaPreview | null>(null)
...
const canSave = computed(() =>
  form.value && form.value.meetingType.trim().length > 0 && form.value.content.trim().length > 0
)
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="$emit('close')">
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-xl font-semibold text-foreground">Agenda saved!</h3>
        <p class="text-sm text-muted-foreground text-center max-w-sm">
          Your meeting agenda has been saved to the Notebook.
        </p>
        <button
          @click="$emit('close')"
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Preparing your agenda...</h3>
        <p class="text-sm text-muted-foreground">Pulling the agenda from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Could not find agenda</h3>
        <p class="text-sm text-muted-foreground">{{ saveError || 'Make sure the agenda has been fully generated in the conversation.' }}</p>
      ...
      <div v-else-if="form" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <p class="text-sm text-muted-foreground">
          Review your agenda before saving. This was extracted from your conversation.
        </p>

        <!-- Meeting Type -->
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-foreground">Meeting Type *</label>
          <input
        ...
        <div class="space-y-1.5">
          ...
          <div class="rounded-lg border border-border bg-muted/30 p-4 max-h-64 overflow-y-auto prose prose-sm prose-slate max-w-none prose-headings:mt-3 prose-headings:mb-1 prose-h2:text-sm prose-h2:font-semibold prose-h3:text-xs prose-h3:font-semibold prose-p:my-1 prose-p:text-xs prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-strong:text-foreground">
            ...
            <p v-else class="text-xs text-muted-foreground italic">No agenda content found.</p>
          </div>
        </div>
      </div>
