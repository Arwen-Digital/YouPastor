...
import type { DevotionalPreview, SaveStatus } from '@/composables/useSaveDevotional'

marked.setOptions({ breaks: true, gfm: true })

const props = defineProps<{
  saveStatus: SaveStatus
  preview: DevotionalPreview | null
...
const emit = defineEmits<{
  (e: 'save', data: DevotionalPreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()

const form = ref<DevotionalPreview | null>(null)
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="$emit('close')">
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-xl font-semibold text-foreground">Devotional saved!</h3>
        <p class="text-sm text-muted-foreground text-center max-w-sm">
          Your midweek devotional has been saved to the Notebook.
        </p>
        <button
          @click="$emit('close')"
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Preparing your devotional...</h3>
        <p class="text-sm text-muted-foreground">Pulling the devotional from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Could not find devotional</h3>
        <p class="text-sm text-muted-foreground">{{ saveError || 'Make sure the devotional has been fully generated in the conversation.' }}</p>
      ...
      <div v-else-if="form" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <p class="text-sm text-muted-foreground">
          Review your devotional before saving. This was extracted from your conversation.
        </p>
