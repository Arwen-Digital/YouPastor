...
import type { YoutubePreview, SaveStatus } from '@/composables/useSaveYoutube'
...
const emit = defineEmits<{
  (e: 'save', data: YoutubePreview): void
  (e: 'close'): void
  (e: 'retry'): void
}>()

const form = ref<YoutubePreview | null>(null)
    ...
    <div class="bg-background border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
      <div class="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        <div class="flex items-center gap-2.5">
          ...
          <h2 class="text-lg font-semibold text-foreground">Save YouTube Package to Notebook</h2>
        </div>
      ...
      <div v-if="saveStatus === 'saved' && savedId" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <h3 class="text-xl font-semibold text-foreground">YouTube package saved!</h3>
        <p class="text-sm text-muted-foreground text-center max-w-sm">Your sermon-to-YouTube output has been saved to the Notebook.</p>
      ...
      <div v-else-if="saveStatus === 'extracting'" class="flex-1 flex flex-col items-center justify-center py-16 px-6 space-y-4">
        ...
        <h3 class="text-lg font-semibold text-foreground">Preparing your YouTube package...</h3>
        <p class="text-sm text-muted-foreground">Pulling the final output from your conversation.</p>
      </div>
      ...
      <div v-else-if="saveStatus === 'error' && !preview" class="flex-1 flex flex-col items-center justify-center py-12 px-6 space-y-4">
        ...
        <p class="text-sm text-muted-foreground text-center">{{ saveError || 'Make sure the final YouTube package has been generated in the conversation.' }}</p>
      ...
      <div v-else-if="form" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <p class="text-sm text-muted-foreground">Review your YouTube package before saving.</p>

        <div class="space-y-1.5">
          <label class="text-sm font-medium text-foreground">Package Title *</label>
          <input
            v-model="form.title"
            type="text"
            placeholder="e.g. Sermon to YouTube Package"
        ...
        <div class="space-y-1.5">
          ...
          <div class="rounded-lg border border-border bg-muted/30 p-4 max-h-80 overflow-y-auto prose prose-sm prose-slate max-w-none prose-headings:mt-3 prose-headings:mb-1 prose-h2:text-sm prose-h2:font-semibold prose-h3:text-xs prose-h3:font-semibold prose-p:my-1 prose-p:text-xs prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-strong:text-foreground">
            ...
            <p v-else class="text-xs text-muted-foreground italic">No output content found.</p>
          </div>
        </div>
      </div>
