...
// Announcement script save flow
const {
  status: announcementSaveStatus,
  preview: announcementPreview,
  error: announcementSaveError,
  savedAnnouncementId,
  extract: extractAnnouncement,
...
const genericIntakePrompt = computed(() => {
  const ctx = churchContext.value
  const ctxBlock = (ctx.churchName || ctx.pastorName)
    ? `\n\n---\n\n${buildContextBlock(ctx)}`
    : ''

  return `You are the orchestrator intake assistant for the ${props.title} skill.

Your job is to gather the minimum information needed before the final deliverable is generated by the final-generation model.

Rules:
- Ask exactly one question per message.
- Do not ask compound or multi-part questions.
- Keep intake turns brief and conversational.
- Do not produce the final deliverable during intake.
- Use the skill instructions below only to decide what information to collect.
- When you have enough information, respond with a message that starts exactly with "SKILL_READY:" followed by a short summary of the collected inputs.

Skill instructions for intake context:
${baseSystemPrompt.value}${ctxBlock}`
})
...
watch(baseSystemPrompt, (newPrompt) => {
  // During intake, specialized/generic intake prompts own currentSystemPrompt.
  if (isIntakePhase.value) return
...
watch(intakePrompt, (newPrompt) => {
  ...
  if (isIntakePhase.value && isSermonResearch) {
    currentSystemPrompt.value = newPrompt
  }
})
...
watch(brainstormIntakePrompt, (newPrompt) => {
  if (isIntakePhase.value && isSermonBrainstorm) {
    currentSystemPrompt.value = newPrompt
  }
})

watch(youtubeIntakePrompt, (newPrompt) => {
  if (isIntakePhase.value && isSermonToYoutube) {
    currentSystemPrompt.value = newPrompt
  }
})

watch(genericIntakePrompt, (newPrompt) => {
  if (isIntakePhase.value && !isSermonResearch && !isSermonBrainstorm && !isSermonToYoutube) {
    currentSystemPrompt.value = newPrompt
  }
})
...
const canSave = computed(() => {
  ...
  const intakeBypassThreshold = isSeriesPlanner ? 6 : isSermonToBlog ? 3 : 4
  const intakeBypass = (isGenericContentSkill || isSeriesPlanner) && assistantMsgs >= intakeBypassThreshold
...
function looksLikeFinalSocialDeliverable(content: string): boolean {
  ...
  // Announcement Script common final structure hints
  const hasAnnouncementSections =
    text.includes('items making the spoken script') ||
    text.includes('items bumped to other channels') ||
    text.includes('spoken script') ||
    text.includes('bumped items') ||
...
async function handleAssistantResponse(responseContent: string, model?: string) {
  ...
  // Detect brainstorm handoff and generate the brief with the researcher model
  ...
  if (isSermonToYoutube && isIntakePhase.value && responseContent.includes('YOUTUBE_READY:')) {
  ...
  if ((isChurchSocialPost || isSocialMediaCalendar || isChurchEmail || isAnnouncementScript || isChurchLetter || isSermonToBlog || isSeriesPlanner) && isIntakePhase.value) {
    const assistantMsgs = messages.value.filter(m => m.role === 'assistant').length
    const fallbackThreshold = isSeriesPlanner ? 6 : isSermonToBlog ? 3 : 4
...
/** Switch from orchestrator intake to researcher model for sermon brainstorm generation */
...
/** Switch from generic orchestrator intake to generator model for final output */
async function handoffToGenerator() {
  isIntakePhase.value = false
  setRole('generator')
  currentSystemPrompt.value = baseSystemPrompt.value

  const triggerMsg = `Please generate the complete ${props.title} deliverable now.`
  messages.value.push({ role: 'user', content: triggerMsg })
  scrollToBottom()
...
function handleSaveConfirmAnnouncement(data: any) {
...
function handleSaveModalClose() {
  ...
  } else if (isAnnouncementScript && announcementSaveStatus.value === 'saved' && savedAnnouncementId.value) {
...
<template>
  <div :class="['flex flex-col', props.embedded ? 'h-full' : 'h-[calc(100vh)]']">
    <div v-if="!props.embedded" class="flex items-center gap-3 border-b px-4 py-3 bg-background shrink-0">
      ...
      <button
        ...
        :class="[
          ...
          canSave
            ? 'bg-violet-600 text-white hover:bg-violet-500 shadow-sm ring-1 ring-violet-700/20'
            : 'bg-violet-100 text-violet-400 border border-violet-200 dark:bg-violet-950/30 dark:text-violet-500/50 dark:border-violet-950/50 cursor-not-allowed',
        ]"
      >
    ...
    <SaveAnnouncementModal
      v-if="showSaveModal && isAnnouncementScript"
      :save-status="announcementSaveStatus"
      :preview="announcementPreview"
      :save-error="announcementSaveError"
...
<style scoped>
.assistant-message {
  animation: assistant-fade-in 180ms ease-out;
}
