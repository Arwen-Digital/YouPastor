...
import { BookOpen, ArrowLeft, ChevronRight, Trash2, Loader2, Search, Heart, CalendarDays, Video, Download } from 'lucide-vue-next'
...
// Which view: 'list', 'series', 'research', 'brainstorm', 'agenda', 'devotional', 'blog', 'youtube', 'smallGroup', 'churchSocial', 'socialCalendar', 'churchEmail', 'announcement', 'churchLetter'
const view = computed(() => {
  if (!route.params.id && !route.params.researchId && !route.params.brainstormId && !route.params.agendaId && !route.params.devotionalId && !route.params.blogId && !route.params.youtubeId && !route.params.smallGroupId && !route.params.churchSocialId && !route.params.calendarId && !route.params.churchEmailId && !route.params.announcementId && !route.params.churchLetterId) return 'list'
  ...
  if (route.params.announcementId) return 'announcement'
  if (route.params.churchLetterId) return 'churchLetter'
...
const selectedAnnouncementId = computed(() => route.params.announcementId as string | undefined)
const selectedChurchLetterId = computed(() => route.params.churchLetterId as string | undefined)

async function openExternalLink(url: string) {
  try {
    await window.appLinks?.openExternal?.(url)
  } catch (err) {
    console.warn('[notebook] Failed to open external link', err)
...
// ── Church social posts list ──
const churchSocialList = ref<any[]>([])
let churchSocialListUnsub: (() => void) | null = null
...
// ── Announcement script list ──
const announcementList = ref<any[]>([])
let announcementListUnsub: (() => void) | null = null

// ── Church letter list ──
const churchLetterList = ref<any[]>([])
let churchLetterListUnsub: (() => void) | null = null
...
// ── Church social detail ──
const churchSocialDetail = ref<any>(null)
const churchSocialDetailLoading = ref(false)
let churchSocialDetailUnsub: (() => void) | null = null

// ── Social media calendar detail ──
...
// ── Announcement script detail ──
const announcementDetail = ref<any>(null)
const announcementDetailLoading = ref(false)
let announcementDetailUnsub: (() => void) | null = null

// ── Church letter detail ──
const churchLetterDetail = ref<any>(null)
const churchLetterDetailLoading = ref(false)
let churchLetterDetailUnsub: (() => void) | null = null
...
onMounted(async () => {
  try {
    ...
    churchSocialListUnsub = client.onUpdate('churchSocial/queries:listMine' as any, {}, (data: any) => {
      churchSocialList.value = data ?? []
    })
    ...
    announcementListUnsub = client.onUpdate('announcement/queries:listMine' as any, {}, (data: any) => {
      announcementList.value = data ?? []
    })
    churchLetterListUnsub = client.onUpdate('churchLetter/queries:listMine' as any, {}, (data: any) => {
      churchLetterList.value = data ?? []
    })
  } catch (err) {
...
onUnmounted(() => {
  ...
  announcementListUnsub?.()
  churchLetterListUnsub?.()
  detailUnsub?.()
  researchDetailUnsub?.()
  ...
  announcementDetailUnsub?.()
  churchLetterDetailUnsub?.()
  seriesResearchUnsub?.()
...
watch(selectedChurchSocialId, async (newId) => {
  if (churchSocialDetailUnsub) { churchSocialDetailUnsub(); churchSocialDetailUnsub = null }
  churchSocialDetail.value = null
  if (!newId) return
  churchSocialDetailLoading.value = true
  try {
    const client = getConvexClient()
    churchSocialDetailUnsub = client.onUpdate('churchSocial/queries:getById' as any, { postId: newId }, (data: any) => {
      churchSocialDetail.value = data
      churchSocialDetailLoading.value = false
    })
  } catch (err) {
    console.error('Failed to load church social post:', err)
    churchSocialDetailLoading.value = false
  }
}, { immediate: true })

watch(selectedCalendarId, async (newId) => {
  if (socialCalendarDetailUnsub) { socialCalendarDetailUnsub(); socialCalendarDetailUnsub = null }
  socialCalendarDetail.value = null
  if (!newId) return
  socialCalendarDetailLoading.value = true
  try {
    const client = getConvexClient()
    socialCalendarDetailUnsub = client.onUpdate('socialCalendar/queries:getById' as any, { calendarId: newId }, (data: any) => {
      socialCalendarDetail.value = data
      socialCalendarDetailLoading.value = false
    })
  } catch (err) {
    console.error('Failed to load social media calendar:', err)
    socialCalendarDetailLoading.value = false
  }
}, { immediate: true })
...
watch(selectedAnnouncementId, async (newId) => {
  if (announcementDetailUnsub) { announcementDetailUnsub(); announcementDetailUnsub = null }
  announcementDetail.value = null
  if (!newId) return
  announcementDetailLoading.value = true
  try {
    const client = getConvexClient()
    announcementDetailUnsub = client.onUpdate('announcement/queries:getById' as any, { announcementId: newId }, (data: any) => {
      announcementDetail.value = data
      announcementDetailLoading.value = false
    })
  } catch (err) {
    console.error('Failed to load announcement script:', err)
    announcementDetailLoading.value = false
  }
}, { immediate: true })

watch(selectedChurchLetterId, async (newId) => {
  if (churchLetterDetailUnsub) { churchLetterDetailUnsub(); churchLetterDetailUnsub = null }
  churchLetterDetail.value = null
  if (!newId) return
  churchLetterDetailLoading.value = true
  try {
    const client = getConvexClient()
    churchLetterDetailUnsub = client.onUpdate('churchLetter/queries:getById' as any, { letterId: newId }, (data: any) => {
      churchLetterDetail.value = data
      churchLetterDetailLoading.value = false
    })
  } catch (err) {
    console.error('Failed to load church letter:', err)
    churchLetterDetailLoading.value = false
  }
}, { immediate: true })

async function handleDelete(type: 'series' | 'research' | 'brainstorm' | 'agenda' | 'devotional' | 'blog' | 'youtube' | 'smallGroupQuestions' | 'churchSocialPost' | 'socialMediaCalendar' | 'churchEmail' | 'announcementScript' | 'churchLetter', id: string) {
  ...
  try {
    ...
    } else if (type === 'churchSocialPost') {
      await client.mutation('churchSocial/mutations:remove' as any, { postId: id })
      if (selectedChurchSocialId.value === id) router.push('/notebook')
    ...
    } else if (type === 'announcementScript') {
      await client.mutation('announcement/mutations:remove' as any, { announcementId: id })
      if (selectedAnnouncementId.value === id) router.push('/notebook')
    } else if (type === 'churchLetter') {
      await client.mutation('churchLetter/mutations:remove' as any, { letterId: id })
      if (selectedChurchLetterId.value === id) router.push('/notebook')
    } else {
...
function slugifyFilePart(value: string | undefined): string {
  return (value || 'research-note')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '')
    .slice(0, 60) || 'research-note'
}

async function downloadResearchPdf() {
  if (!researchDetail.value || !researchDetail.value.content || exportingResearchPdf.value) return

  exportingResearchPdf.value = true
  try {
    ...
    const title = researchDetail.value.scriptureRef || 'Research Note'
    drawText(title, { size: 18, bold: true, gapAfter: 8 })

    if (researchDetail.value.topicOrAngle) {
      drawText(researchDetail.value.topicOrAngle, { size: 12, gapAfter: 8 })
    }

    const linkedSeries = researchDetail.value.seriesId ? getSeriesName(researchDetail.value.seriesId) : ''
    const meta = [
      researchDetail.value.createdAt ? `Created ${formatDate(researchDetail.value.createdAt)}` : '',
    ...
    const lines = String(researchDetail.value.content || '').replace(/\r\n/g, '\n').split('\n')
...
function getTypeBadge(type: string): { label: string; color: string } {
  switch (type) {
    ...
    case 'announcementScript': return { label: 'Announcement', color: 'bg-lime-500/10 text-lime-700' }
    case 'churchLetter': return { label: 'Church Letter', color: 'bg-fuchsia-500/10 text-fuchsia-700' }
...
const combinedList = computed(() => {
  const items: Array<{
    type: 'sermon' | 'series' | 'research' | 'brainstorm' | 'agenda' | 'devotional' | 'blog' | 'youtube' | 'smallGroupQuestions' | 'churchSocialPost' | 'socialMediaCalendar' | 'churchEmail' | 'announcementScript' | 'churchLetter'
    id: string
    title: string
    subtitle: string
    date: number
    ...
    ...announcementList.value.map(a => ({ type: 'announcementScript' as const, id: a._id, title: a.title || 'Announcement Script', subtitle: '', date: a.createdAt || 0, status: a.status || 'draft', tab: 'content' as const })),
    ...churchLetterList.value.map(l => ({ type: 'churchLetter' as const, id: l._id, title: l.title || 'Church Letter', subtitle: '', date: l.createdAt || 0, status: l.status || 'draft', tab: 'content' as const })),
  ]
...
<template>
  <div class="flex flex-col h-[calc(100vh)]">
      ...
      <div class="max-w-4xl mx-auto flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-muted-foreground">
            ...
            <template v-else-if="view === 'announcement'">Announcement Script</template>
            <template v-else-if="view === 'churchLetter'">Church Letter</template>
        ...
        <div v-else class="space-y-3">
          ...
          <button
            ...
            @click="router.push(
              ...
              : item.type === 'churchEmail' ? `/notebook/church-email/${item.id}`
          ...
          >
            <div class="mt-0.5 shrink-0">
              <div :class="['h-10 w-10 rounded-lg flex items-center justify-center', getTypeBadge(item.type).color]">
                ...
                <svg v-else-if="item.type === 'announcementScript'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11v3a1 1 0 0 0 1 1h3"/><path d="M7 11l10-5v12L7 13"/><path d="M11 14v4a1 1 0 0 0 1 1h2"/></svg>
      ...
      <div class="max-w-4xl mx-auto px-6 py-6">
        ...
        <div v-else-if="researchDetail" class="space-y-6">
          ...
          <div v-if="researchDetail.content" class="rounded-lg border border-border bg-card p-6 prose prose-sm prose-slate max-w-none prose-headings:mt-6 prose-headings:mb-3 prose-h2:text-lg prose-h2:font-semibold prose-h3:text-base prose-h3:font-semibold prose-p:my-2 prose-p:text-sm prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-li:text-sm prose-strong:text-foreground prose-table:my-3 prose-table:text-sm prose-th:px-3 prose-th:py-2 prose-th:bg-muted/50 prose-th:font-medium prose-td:px-3 prose-td:py-1.5 prose-td:border-t prose-td:border-border prose-code:text-xs prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded">
            <div @click="handleMarkdownClick" v-html="marked.parse(researchDetail.content)" />
          </div>
    ...
    <!-- Church Social Detail View -->
    <div v-else-if="view === 'churchSocial'" class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto px-6 py-6">
        <div v-if="churchSocialDetailLoading" class="flex items-center justify-center py-12">
          <Loader2 class="h-6 w-6 animate-spin text-muted-foreground" />
        </div>

        <div v-else-if="churchSocialDetail" class="space-y-6">
          <div class="flex items-start justify-between gap-4">
            <div class="space-y-1 min-w-0">
              <div class="flex items-center gap-2">
                <h2 class="text-2xl font-semibold tracking-tight text-foreground">{{ churchSocialDetail.title }}</h2>
                <span class="text-xs px-2 py-0.5 rounded-full font-medium bg-pink-100 text-pink-700">Church Social</span>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <button
                @click="handleDelete('churchSocialPost', churchSocialDetail._id)"
                :disabled="deletingId === churchSocialDetail._id"
                class="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-50"
              >
                <Trash2 class="h-3.5 w-3.5" />
                Delete
          ...
          <div v-if="churchSocialDetail.content" class="rounded-lg border border-border bg-card p-6 prose prose-sm prose-slate max-w-none prose-headings:mt-6 prose-headings:mb-3 prose-h2:text-lg prose-h2:font-semibold prose-h3:text-base prose-h3:font-semibold prose-p:my-2 prose-p:text-sm prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-li:text-sm prose-strong:text-foreground prose-blockquote:my-3 prose-blockquote:rounded-lg prose-blockquote:border-l-4 prose-blockquote:border-pink-300 prose-blockquote:bg-pink-50/60 prose-blockquote:px-4 prose-blockquote:py-3 prose-blockquote:not-italic prose-blockquote:text-slate-700">
            <div v-html="marked.parse(churchSocialDetail.content)" />
          </div>

          <div class="flex items-center gap-4 text-xs text-muted-foreground pt-4 border-t border-border">
            <span v-if="churchSocialDetail.createdAt">Created {{ formatDate(churchSocialDetail.createdAt) }}</span>
            <span v-if="churchSocialDetail.updatedAt && churchSocialDetail.updatedAt !== churchSocialDetail.createdAt">Updated {{ formatDate(churchSocialDetail.updatedAt) }}</span>
          </div>
        </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 space-y-4">
          <div class="h-16 w-16 rounded-2xl bg-muted flex items-center justify-center"><BookOpen class="h-8 w-8 text-muted-foreground" /></div>
          <div class="text-center space-y-1">
            <h3 class="text-lg font-semibold text-foreground">Church Social Post not found</h3>
            <p class="text-sm text-muted-foreground">This post may have been deleted.</p>
          </div>
          <button @click="router.push('/notebook')" class="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">Back to Notebook</button>
        </div>
      </div>
    ...
    <!-- Social Media Calendar Detail View -->
    <div v-else-if="view === 'socialCalendar'" class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto px-6 py-6">
        <div v-if="socialCalendarDetailLoading" class="flex items-center justify-center py-12">
          <Loader2 class="h-6 w-6 animate-spin text-muted-foreground" />
        </div>

        <div v-else-if="socialCalendarDetail" class="space-y-6">
          <div class="flex items-start justify-between gap-4">
            <div class="space-y-1 min-w-0">
              <div class="flex items-center gap-2">
                <h2 class="text-2xl font-semibold tracking-tight text-foreground">{{ socialCalendarDetail.title }}</h2>
                <span class="text-xs px-2 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-700">Social Calendar</span>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <button
                @click="handleDelete('socialMediaCalendar', socialCalendarDetail._id)"
                :disabled="deletingId === socialCalendarDetail._id"
                class="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-50"
              >
                <Trash2 class="h-3.5 w-3.5" />
                Delete
          ...
          <div v-if="socialCalendarDetail.content" class="rounded-lg border border-border bg-card p-6 overflow-x-auto prose prose-sm prose-slate max-w-none prose-headings:mt-6 prose-headings:mb-3 prose-h2:text-lg prose-h2:font-semibold prose-h3:text-base prose-h3:font-semibold prose-p:my-2 prose-p:text-sm prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-li:text-sm prose-strong:text-foreground prose-table:my-3 prose-table:text-sm prose-th:px-3 prose-th:py-2 prose-th:bg-muted/50 prose-th:font-medium prose-td:px-3 prose-td:py-1.5 prose-td:border-t prose-td:border-border">
            <div v-html="marked.parse(socialCalendarDetail.content)" />
          </div>

          <div class="flex items-center gap-4 text-xs text-muted-foreground pt-4 border-t border-border">
            <span v-if="socialCalendarDetail.createdAt">Created {{ formatDate(socialCalendarDetail.createdAt) }}</span>
            <span v-if="socialCalendarDetail.updatedAt && socialCalendarDetail.updatedAt !== socialCalendarDetail.createdAt">Updated {{ formatDate(socialCalendarDetail.updatedAt) }}</span>
          </div>
        </div>

        <div v-else class="flex flex-col items-center justify-center py-20 space-y-4">
          <div class="h-16 w-16 rounded-2xl bg-muted flex items-center justify-center"><CalendarDays class="h-8 w-8 text-muted-foreground" /></div>
          <div class="text-center space-y-1">
    ...
    <!-- Announcement Script Detail View -->
    <div v-else-if="view === 'announcement'" class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto px-6 py-6">
        <div v-if="announcementDetailLoading" class="flex items-center justify-center py-12">
          <Loader2 class="h-6 w-6 animate-spin text-muted-foreground" />
        </div>

        <div v-else-if="announcementDetail" class="space-y-6">
          <div class="flex items-start justify-between gap-4">
            <div class="space-y-1 min-w-0">
              <div class="flex items-center gap-2">
                <h2 class="text-2xl font-semibold tracking-tight text-foreground">{{ announcementDetail.title }}</h2>
                <span class="text-xs px-2 py-0.5 rounded-full font-medium bg-lime-100 text-lime-700">Announcement</span>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <button
                @click="handleDelete('announcementScript', announcementDetail._id)"
                :disabled="deletingId === announcementDetail._id"
                class="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-50"
              >
                <Trash2 class="h-3.5 w-3.5" />
                Delete
          ...
          <div v-if="announcementDetail.content" class="rounded-lg border border-border bg-card p-6 prose prose-sm prose-slate max-w-none prose-headings:mt-6 prose-headings:mb-3 prose-h2:text-lg prose-h2:font-semibold prose-h3:text-base prose-h3:font-semibold prose-p:my-2 prose-p:text-sm prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-li:text-sm prose-strong:text-foreground">
            <div v-html="marked.parse(announcementDetail.content)" />
          </div>

          <div class="flex items-center gap-4 text-xs text-muted-foreground pt-4 border-t border-border">
            <span v-if="announcementDetail.createdAt">Created {{ formatDate(announcementDetail.createdAt) }}</span>
            <span v-if="announcementDetail.updatedAt && announcementDetail.updatedAt !== announcementDetail.createdAt">Updated {{ formatDate(announcementDetail.updatedAt) }}</span>
          </div>
        </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 space-y-4">
          <div class="h-16 w-16 rounded-2xl bg-muted flex items-center justify-center"><BookOpen class="h-8 w-8 text-muted-foreground" /></div>
          <div class="text-center space-y-1">
            <h3 class="text-lg font-semibold text-foreground">Announcement Script not found</h3>
            <p class="text-sm text-muted-foreground">This script may have been deleted.</p>
          </div>
          <button @click="router.push('/notebook')" class="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">Back to Notebook</button>
        </div>
      </div>
    ...
    <!-- Church Letter Detail View -->
    <div v-else-if="view === 'churchLetter'" class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto px-6 py-6">
        <div v-if="churchLetterDetailLoading" class="flex items-center justify-center py-12">
          <Loader2 class="h-6 w-6 animate-spin text-muted-foreground" />
        </div>

        <div v-else-if="churchLetterDetail" class="space-y-6">
          <div class="flex items-start justify-between gap-4">
            <div class="space-y-1 min-w-0">
              <div class="flex items-center gap-2">
                <h2 class="text-2xl font-semibold tracking-tight text-foreground">{{ churchLetterDetail.title }}</h2>
                <span class="text-xs px-2 py-0.5 rounded-full font-medium bg-fuchsia-100 text-fuchsia-700">Church Letter</span>
              </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <button
                @click="handleDelete('churchLetter', churchLetterDetail._id)"
                :disabled="deletingId === churchLetterDetail._id"
                class="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-50"
              >
                <Trash2 class="h-3.5 w-3.5" />
                Delete
          ...
          <div v-if="churchLetterDetail.content" class="rounded-lg border border-border bg-card p-6 prose prose-sm prose-slate max-w-none prose-headings:mt-6 prose-headings:mb-3 prose-h2:text-lg prose-h2:font-semibold prose-h3:text-base prose-h3:font-semibold prose-p:my-2 prose-p:text-sm prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1 prose-li:text-sm prose-strong:text-foreground">
            <div v-html="marked.parse(churchLetterDetail.content)" />
          </div>

          <div class="flex items-center gap-4 text-xs text-muted-foreground pt-4 border-t border-border">
            <span v-if="churchLetterDetail.createdAt">Created {{ formatDate(churchLetterDetail.createdAt) }}</span>
            <span v-if="churchLetterDetail.updatedAt && churchLetterDetail.updatedAt !== churchLetterDetail.createdAt">Updated {{ formatDate(churchLetterDetail.updatedAt) }}</span>
          </div>
        </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 space-y-4">
          <div class="h-16 w-16 rounded-2xl bg-muted flex items-center justify-center"><BookOpen class="h-8 w-8 text-muted-foreground" /></div>
          <div class="text-center space-y-1">
            <h3 class="text-lg font-semibold text-foreground">Church Letter not found</h3>
            <p class="text-sm text-muted-foreground">This letter may have been deleted.</p>
          </div>
          <button @click="router.push('/notebook')" class="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">Back to Notebook</button>
        </div>
      </div>
