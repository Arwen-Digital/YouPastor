...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("blogPosts", {
      userId,
      title: args.title.trim() || "Sermon Blog Post",
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!existing || existing.userId !== userId) throw new Error("Blog post not found")

    const patch: any = { updatedAt: Date.now() }
    if (args.title !== undefined) patch.title = args.title.trim() || "Sermon Blog Post"
